...
function behaviorBlank(){return{decisions:0,spawns:0,waits:0,lane:[0,0,0],roles:{},spent:0,goldSum:0,goldSamples:0,laneSwitches:0,lastLane:null,adaptations:0,trace:{},traceN:0}}
...
function mergeBehavior(dst,b,winner,side){
 ...
 for(const k of ['decisions','spawns','waits','spent','goldSum','goldSamples','laneSwitches','adaptations'])dst[k]=(dst[k]||0)+(b[k]||0);
...
function behaviorText(s){
 ...
 return `Reforços: sup ${lane[0]}% • centro ${lane[1]}% • inf ${lane[2]}%. ${roles||'sem compras'}. Ouro médio retido ${Math.round(gold)}. Esperou em ${Math.round(wait*100)}% das decisões; mudou de frente em ${Math.round(sw*100)}%. Ajustes internos ${s.adaptations||0}.`
}
...
run=async function(){
 let total=Number($('#matchCount').value),n=Number($('#candidateCount').value),cands=generateCandidates(n),stats={},strategyStats={},durations=blankDurations(),bar=$('#progressBar'),st=$('#labStatus'),btn=$('#runLab');
 cands.forEach(policyFor);btn.disabled=true;bar.style.width='0';st.textContent='Criando políticas e preparando agentes...';$('#bestComps').innerHTML='';$('#unitTable').textContent='Simulando decisões estratégicas...';
 for(let m=0;m<total;m++){
  let i=Math.floor(Math.random()*cands.length),j=Math.floor(Math.random()*(cands.length-1));if(j>=i)j++;let a=cands[i],b=cands[j],r=simMatch(a,b);
  addDuration(durations,r);updateCandidate(a,r,1);updateCandidate(b,r,-1);mergeStats(stats,r.A,r.winner,1);mergeStats(stats,r.B,r.winner,-1);
  mergeBehavior(strategyStats[a.id]||(strategyStats[a.id]={}),r.behaviorA,r.winner,1);mergeBehavior(strategyStats[b.id]||(strategyStats[b.id]={}),r.behaviorB,r.winner,-1);
  if(m%25===0){bar.style.width=(m/total*100).toFixed(1)+'%';st.textContent=`${m.toLocaleString('pt-BR')} / ${total.toLocaleString('pt-BR')} partidas estratégicas`;await new Promise(requestAnimationFrame)}
 }
 bar.style.width='100%';renderTop(cands);renderUnits(stats);$('#durationCurrent').innerHTML=durationHTML(durations);saveDatabase(stats,total,durations);snapshot(cands,stats,total,durations);saveStrategyBatch(total,cands,strategyStats);renderDatabase();renderHistory();renderStrategies(cands,strategyStats);
